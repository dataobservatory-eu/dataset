...
In the abstract, and following ISO/IEC 20546, a dataset is an identifiable collection of data available for access or download in one or more formats. This is different from R’s `data.frame` object, which is one specific in-memory structure. While R can already serialise data frames to formats like `.rds`, `.rda`, or `.csv`, these serialisations lack rich, standardised metadata. The `dataset` package aims to bridge that gap by aligning with established metadata standards to produce serialisations that are easier to reuse and interpret.

`dataset_df` works naturally with data structured according to tidy data principles (Wickham, 2014), where each variable is a column, each observation is a row, and each type of observational unit forms a table. It adds an additional semantic layer to these tidy structures, so the meaning of variables and datasets is explicit and machine-readable.

This vignette focuses specifically on the `dataset_df` class and the `dataset_df()` constructor, which you can use to create a semantically enriched data frame. For details on semantically enriched vectors, see `vignette("defined", package = "dataset")`.

## Purpose

The `dataset_df()` function helps you create **semantically rich datasets** that are easier to:
