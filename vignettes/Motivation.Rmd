...
> "A dataset is a structured, coherent collection of related data, organised for a particular purpose." ISO 19115 and ISO/IEC 11179
...
Specifically, `dataset` utilizes `person` and extended `bibentry` objects (from the `utils` package) within the attributes. This allows comprehensive descriptive metadata to be associated with the dataset. The package focuses on adhering to either DCTERMS (Dublin Core) or DataCite metadata standards, maintaining and even updating this metadata throughout the R workflow.
...
named_triples <- combined_subject %>%
  pivot_longer(cols = c(year, country_name, GDP), # geocode is excluded
               names_to = "predicate",
               values_to = "object")
...
geonames_mapping <- tibble(
  ...
  geonames_id = c("6252001", "6251999", "2988317") # Example Geonames IDs - REPLACE THESE
)
...
rdf_triples <- named_triples %>%
  ...
  mutate(
    ...
    object = case_when(
      predicate == "sdmx:TIME_PERIOD" ~ paste0("\"", object, "\"^^<http://www.w3.org/2001/XMLSchema#gYear>"), # Year with datatype
      predicate == "sdmx:OBS_VALUE" ~ paste0("\"", object, "\"^^<http://www.w3.org/2001/XMLSchema#double>"), # GDP with datatype
...
D <- tibble(
  ...
  geocode = c("USA", "CAN", "USA", "CAN", "USA", "CAN"),
  country_name = c("United States", "Canada", "United States", "Canada", "United States", "Canada"),
  GDP = c(21000000, 2000000, 22000000, 2100000, 23000000, 2200000) # GDP in millions of USD
)
...
```{r}
# Create E_defined
geonames_ids_E <- rep(c(geonames_ids[1], geonames_ids[3]), each = 3) #USA and France
...
Let us go back to the ISO 19115 and ISO/IEC 11179 definition used as a motto: "A dataset is a structured, coherent collection of related data, organised for a particular purpose." 
