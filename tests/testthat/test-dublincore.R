test_that("fix_publisher() works", {
  ...
  expect_equal(fix_publisher(publishers = c(person("Jane", "Doe", role = "pbl"),
                                            person("American Iris Society"))), "{Jane} and {American Iris Society}")
})
...
test_that("new_dublincore() works", {
  expect_equal(new_dublincore(title="Test", creator=person("Jane", "Doe", role = "cre"))$author,
               person("Jane", "Doe", role = "cre"))
  expect_equal(new_dublincore(title="Test", creator=person("Jane", "Doe", role = "cre"))$title,
               "Test")
  expect_equal(new_dublincore(title="Test",
                              creator=person("Jane", "Doe", role = "cre"),
                              contributor=c(person("Joe", "Doe", role = "dtm"),
                                            person("Daniel", "Antal", role = "ctb")))$contributor,
               "{Joe Doe [dtm]} and {Daniel Antal [ctb]}")
  expect_equal(new_dublincore(title="Test",
                              creator=person("Jane", "Doe", role = "cre"),
                              publisher=person("My Publisher Inc.", role = "pbl"))$publisher,
              "My Publisher Inc.")
  expect_equal(new_dublincore(title="Test",
                              creator=person("Jane", "Doe", role = "cre"),
                              datasource = "https://doi.org/10.1111/j.1469-1809.1936.tb02137.x")$datasource,
               "https://doi.org/10.1111/j.1469-1809.1936.tb02137.x")
  expect_equal(new_dublincore(title="Test",
                              creator=person("Jane", "Doe", role = "cre"),
                              publication_date= 1935)$date,
               "1935")
  expect_equal(new_dublincore(title="Test",
                              creator=person("Jane", "Doe", role = "cre"),
...
test_that("dublincore works", {
  dct_iris1 <- dublincore(
    title = "Iris Dataset",
    creator = c(person(given = "Edgar", family="Anderson", role = "aut"),
  ...
  expect_equal(dct_iris1$author,  c(person(given = "Edgar", family="Anderson", role = "aut"),
                                    person(given = "Jane D", family="Anderson", role = "cre")))
